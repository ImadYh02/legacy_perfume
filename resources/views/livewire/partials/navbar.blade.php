<header class="flex flex-wrap sm:justify-start sm:flex-nowrap w-full bg-transparent text-sm py-4 dark:bg-neutral-800">
    <nav class="max-w-[85rem] w-full mx-auto px-4 flex flex-wrap basis-full items-center justify-between" aria-label="Global">
        ...
        <div class="sm:order-3 flex items-center gap-x-2">
            {{-- ===== Shop Cart ===== --}}
            <a class="hs-tooltip-toggle hs-tooltip [--placement:left] hover:text-[#FF1493]" aria-haspopup="dialog" aria-expanded="false" aria-controls="hs-offcanvas-right" data-hs-overlay="#hs-offcanvas-right" href="#">
            ...
            {{-- ===== Profile Login ===== --}}
            <a class="sm:ml-4 hs-tooltip-toggle hs-tooltip [--placement:bottom] hover:text-[#FF1493]" wire:navigate href="{{route('filament.admin.auth.login')}}">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
        ...
        <div id="navbar-alignment" class="hs-collapse hidden overflow-hidden transition-all duration-300 basis-full grow sm:grow-0 sm:basis-auto sm:block sm:order-2">
            <div class="flex flex-col gap-5 mt-5 sm:flex-row sm:items-center sm:mt-0 sm:ps-5">
                <a class="font-medium {{ request()->is('/')?'text-[#FF1493]':'text-black' }}" wire:navigate href="/">Home</a>
                <a class="font-medium hover:text-[#FF1493] {{ request()->is('shop')?'text-[#FF1493]':'text-black' }}" wire:navigate href="/shop">Shop</a>
                <a class="font-medium hover:text-[#FF1493] {{ request()->is('track-order')?'text-[#FF1493]':'text-black' }}" wire:navigate href="/track-order">Track Order</a>
                <a class="font-medium hover:text-[#FF1493] {{ request()->is('contact')?'text-[#FF1493]':'text-black' }}" wire:navigate href="/contact">Contact</a>
            </div>
        </div>
    </nav>
